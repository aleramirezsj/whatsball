...
public class ScriptConfiguracion : MonoBehaviour
{
    ...
    void Update()
    {
		//Detectamos el botón atras fisíco del dispositivo
    ...
    void recuperarSeteosJugador()
    {
        ...
        /*JOAQUIN*/
        pelota.transform.localScale = new Vector3(int.Parse(TxtTamanioPelota.text) / 2, int.Parse(TxtTamanioPelota.text) / 2, int.Parse(TxtTamanioPelota.text) / 2);
    }
}
